context('Calculator', () => {
	...
	describe('Uses the calculator', () => {
		...
		it('Checks if all buttons and displays were loaded', () => {
			...
			cy.get('#calculator')
				.find('.display')
				.should('have.length', totalDisplays)
		})
		...
		it('Makes random calculations', () => {
			...
			describe('Select the operator button', () => {
				...
				cy.contains('button', randomOperator).click()

				// Check if the operator is on the "First values" input with the first number
				cy.get('.firstValue').should(
					'have.value',
					firstRandomNumber + randomOperator
				)
			})

			describe('Select the first number button', () => {
				// Press the second random number button
				cy.contains('button', secondRandomNumber).click()

				cy.get('.firstValue').should(
					'have.value',
					firstRandomNumber + randomOperator + secondRandomNumber
				)
			})
			...
			describe('Uses the currently data given until now', () => {
				...
				function finalResult() {
					...
					case '+':
						value = firstRandomNumber + secondRandomNumber
						break
